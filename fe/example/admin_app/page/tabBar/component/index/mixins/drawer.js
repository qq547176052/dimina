...
//   2026-07-25 退出登录修正: 清空 token+用户名+密码+user(回到默认空对象), 且 await 宿主清空成功后再 reLaunch 登录页, 杜绝异步回写污染导致登录页自动登录弹回首页
...
module.exports = {
  ...
  // 是否保存(记住账号开关, 非登录态)保持默认 true, 不影响已退出判定; 不销毁小程序(仅 reLaunch 重置页面栈)
  _logout() {
    const app = getApp()
